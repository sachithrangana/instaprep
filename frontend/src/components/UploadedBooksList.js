...
import './UploadedBooksList.css';

function UploadedBooksList({ books, onBookClick, uploadProgress }) {
  ...
  return (
    <div className="uploaded-books-grid">
      {books.map((book) => {
        ...
        // Only show uploading if this specific book is being uploaded (by ID match)
        // Don't show on all books when general upload is in progress
        const isUploading = uploadProgress && uploadProgress.uploadingBookId === book.id;
        // Check if book is in indexing state (pending, queued, or processing)
        const isIndexing = book.status === 'pending' || book.status === 'queued' || book.status === 'processing';
        const showIndexing = isIndexing && !isUploading;
        
        return (
          <div
            key={book.id}
            className={`uploaded-book-card ${!onBookClick ? 'uploaded-book-card-no-click' : ''} ${isUploading ? 'uploading' : ''} ${showIndexing ? 'indexing' : ''}`}
            onClick={onBookClick && !isUploading && !showIndexing ? () => onBookClick(book.book_id) : undefined}
          >
            <div className="book-card-header">
              ...
              {showIndexing && (
                <div className="upload-progress-indicator">
                  <div className="upload-spinner"></div>
                  <span className="upload-status-text">Indexing...</span>
                </div>
              )}
            </div>
            {book.description && !isUploading && !showIndexing && (
              <p className="book-description">{book.description}</p>
            )}
            {isUploading && (
              <p className="upload-progress-message">{uploadProgress?.message || 'Processing and indexing book...'}</p>
            )}
            {showIndexing && (
              <p className="upload-progress-message">Processing and indexing book content...</p>
            )}
            {!isUploading && !showIndexing && (
